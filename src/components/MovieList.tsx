...
import { ToastContainer, Row, Col, Container, Form } from "react-bootstrap";
...
const MovieList = (props: Props) => {
  const [filteredItems, setFilteredItems] = useState<MovieInfo[]>([]);
  const [toast, setToast] = useState<ToastObject[]>([]);
  const [search, setSearch] = useState<string>("");
  const [receivedData, setReceivedData] = useState<MovieInfo[]>([]);
  ...
  const filterItems = (searchString: string, data: MovieInfo[]) => {
    if (searchString !== "") {
  ...
  const handleFavourites = async (movie: MovieInfo) => {
    try {
      if (section === "favourite") {
        ...
        const movieAlreadyPresent = favouriteMovies.filter(
          (data: { title: string }) => data.title === movie.title
        );
